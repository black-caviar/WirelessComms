...
function out = transmit(data, chan, M, SNR, nTrain, eq)
    % Simulate transmitter and receiver 
    bpSym = log2(M); % bits per symbol
    rdata = reshape(data, bpSym, []);
    ...
    txNoisy = awgn(txChan, SNR + 10*log10(bpSym)); % Add AWGN
    
    if isequal(nTrain,0)
        txEQ = txNoisy; % ditto
    else
        txEQ = equalize(eq, txNoisy, tx(1:nTrain));
    end 
    ...
    rxMSG = de2bi(rx); 
    out = reshape(rxMSG', [], 1); %return bit stream
end